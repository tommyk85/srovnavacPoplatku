...
$sql_ucty = "SELECT ucet_nazev, nazev_banky FROM ucty INNER JOIN banky ON ucty.ucet_kod_banky = banky.kod_banky WHERE kod_banky IN ($kody) 
...
while($radek_ucty = mysql_fetch_assoc($ucty))
{
     ...
     <TD nowrap>".$radek_ucty['nazev_banky']."</TD></TR>";
}
?>
...
<?php
$sql_ucty2 = "SELECT ucet_nazev, nazev_banky FROM ucty INNER JOIN banky ON ucty.ucet_kod_banky = banky.kod_banky WHERE kod_banky NOT IN ($kody) 
ORDER BY nazev_banky ASC, ucet_nazev ASC";
...
<?php
$sql_banky = "SELECT * FROM banky WHERE kod_banky IN ($kody) ORDER BY nazev_banky ASC";

...
date_default_timezone_set('Europe/Prague');
  ...
  $set['Zdroj'] = "'" . mysql_real_escape_string($_POST['zdroj']) . "'";
  ...
  if(!$id_vysledku)
  {
    ...
    die('Nepodařilo se nám poslat SQL dotaz na vlozeni prijmu.');
  }
  ...
  <a href="fin_uvod.htm">Na uvodni stranku.</A><P>';
  }
...
if(!$stav_uctu)
  {
    echo mysql_errno($id_spojeni).': '.mysql_error($id_spojeni).'<br>';
    die('Nepodařilo se nám poslat SQL dotaz na stav uctu.');
  }
...
$sql_navyseni_uctu = "UPDATE moje_ucty SET
  ...
  WHERE NazevUctu = '".$_POST['ucet']."'";
  
$navyseni_uctu = mysql_query($sql_navyseni_uctu, $id_spojeni);
if(!$navyseni_uctu)
  {
    echo mysql_errno($id_spojeni).': '.mysql_error($id_spojeni).'<br>';
    die('Nepodařilo se nám poslat SQL dotaz na navyseni zustatku uctu.');
  }

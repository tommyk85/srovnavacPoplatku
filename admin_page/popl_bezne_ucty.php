...
if(isset($_GET['vlozeni_popl'])){                                         //   VLOZENI NOVYCH POPLATKU
    if(trim($_GET['platnostOd'])==="") {
        ...
        (cena_ucet_id, cena_platnost_od, cena_zrizeni, cena_zruseni, cena_vedeni, cena_vedeni_podm, cena_vypis_e, cena_vypis_p, cena_prichozi1, cena_prichozi2, cena_odchozi_tp1, cena_odchozi_tp2, cena_odchozi_online1, cena_odchozi_priorita, cena_inkaso_svoleni, cena_inkaso_odchozi, cena_kontokorent_zrizeni, cena_kontokorent_vedeni, cena_kontokorent_zruseni, cena_koment_JP, cena_koment_PP, cena_koment_trans, cena_koment_inkaso, cena_koment_kontokorent, cena_koment_karta)
        ...
        .($_GET['kontZrizeni'] == "" ? "Null, " : $_GET['kontZrizeni'].", ")
        .($_GET['kontVedeni'] == "" ? "Null, " : $_GET['kontVedeni'].", ")
        .($_GET['kontZruseni'] == "" ? "Null, " : $_GET['kontZruseni'].", ")
        ."'".(htmlspecialchars($_GET['koment_JP'], ENT_QUOTES))."', '".(htmlspecialchars($_GET['koment_PP'], ENT_QUOTES))."', '".(htmlspecialchars($_GET['koment_trans'], ENT_QUOTES))."', '".(htmlspecialchars($_GET['koment_ink'], ENT_QUOTES))."', '".(htmlspecialchars($_GET['koment_kont'], ENT_QUOTES))."', 'bez karty')";
        ...
        if(isset($_GET['vloz_popl_vc_bal']) && $_GET['vloz_popl_vc_bal'] == 1) {
        copyBalicky($_GET['ucet_vzor'], $max_id);
        }

        echo "<meta http-equiv='refresh' content='0;url=/srovnavacPoplatku/admin_page/admin.php?kodBanky=".$_GET['kodBanky']."&ucet=".$_GET['ucet']."&ucetTyp=".$_GET['ucetTyp']."&nazevUctu=".$_GET['nazevUctu']."&id=$max_id&r_cena_d=".$_GET['r_cena_d']."&note=Poplatky vloženy - ID $max_id. Můžeš vložit první kartu.#poplatky'>";
    }
}
...
.($_GET['kontZrizeni'] == Null ? "cena_kontokorent_zrizeni=Null, " : "cena_kontokorent_zrizeni=".$_GET['kontZrizeni'].", ")
.($_GET['kontVedeni'] == Null ? "cena_kontokorent_vedeni=Null, " : "cena_kontokorent_vedeni=".$_GET['kontVedeni'].", ")
.($_GET['kontZruseni'] == Null ? "cena_kontokorent_zruseni=Null, " : "cena_kontokorent_zruseni=".$_GET['kontZruseni'].", ")
...
cena_koment_kontokorent = '".(htmlspecialchars($_GET['koment_kont'], ENT_QUOTES))."'                                                         
...
<H2><?php echo ($_GET['id'] > 0 ? "Výpis poplatků k účtu <U>".$_GET['nazevUctu']."</U>" : "Vložení nových poplatků pro <U>".$_GET['nazevUctu']."</U>"); ?></H2>
<?php 
  ...
  <BR><a href='../admin_page/admin.php?kodBanky=".$_GET['kodBanky']."&ucet=".$_GET['ucet']."&ucetTyp=".$_GET['ucetTyp']."&nazevUctu=".$_GET['nazevUctu']."&id=0&vyber_id=' accesskey='n'>Zadat nové poplatky</a>";
  }
  else {
...
echo "bez vzoru";
$zrizeniUctu_vzor = $zruseniUctu_vzor = $vedeniUctu_vzor = $vypis_vzor = $prichozi1_vzor = $prichozi2_vzor = $odchoziTP1_vzor = $odchoziTP2_vzor = $odchoziOn1_vzor = $odchoziOn2_vzor = $balicek_vzor = $koment_JP_vzor = $koment_PP_vzor = $koment_trans_vzor = $zrizeni_banking_vzor = $vedeni_banking_vzor = $odchozi1_vzor = $odchozi2_vzor = $zrizeniTP_vzor = $odchoziP_vzor = $inkSvoleni_vzor = $inkOdch_vzor = $koment_ink_vzor = $kontZrizeni_vzor = $kontVedeni_vzor = $kontZruseni_vzor = $koment_kont_vzor = Null;
}
?>
<BR>
...
<H3>Kontokorent</H3>
Zřízení: <INPUT <?php echo ($kontZrizeni == "" ? "class='chybi' " : ""); ?>type="number" name="kontZrizeni" value="<?php echo $kontZrizeni; ?>" style="text-align:right; width:60"<?php echo $popl_read_only; ?>>
<?php echo "<span class='help'>$kontZrizeni_vzor</span>"; ?><BR>
Vedení: <INPUT <?php echo ($kontVedeni == "" ? "class='chybi' " : ""); ?>type="number" name="kontVedeni" value="<?php echo $kontVedeni; ?>" style="text-align:right; width:60"<?php echo $popl_read_only; ?>>
<?php echo "<span class='help'>$kontVedeni_vzor</span>"; ?><BR>
Zrušení: <INPUT <?php echo ($kontZruseni == "" ? "class='chybi' " : ""); ?>type="number" name="kontZruseni" value="<?php echo $kontZruseni; ?>" style="text-align:right; width:60"<?php echo $popl_read_only; ?>>
<?php echo "<span class='help'>$kontZruseni_vzor</span>"; ?><BR>

Komentář: <TEXTAREA name="koment_kont" cols=80 rows=6<?php echo $popl_read_only; ?>><?php echo $koment_kont; ?></TEXTAREA> 
<?php echo "<br><span class='help'>$koment_kont_vzor</span>"; ?>
</FORM>
